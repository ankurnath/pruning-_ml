...
if __name__ == "__main__":
    ...
    parser.add_argument("--depth",type=int,default=473)
    ...
    try:
        ...
        gnn_save_folder =  f'pretrained/{problem}/GNNpruner/{dataset}'
        pruner.train(train_graph=train_graph,
                    budget=budget,
                    heuristic=heuristic,
                    save_folder =gnn_save_folder)
        pruner.model.load_state_dict(torch.load(load_model_path,weights_only=False))
...
pruned_universe_gnn = [forward_transformation[node] for node in pruned_universe_gnn]

game  = env(graph=relabel_subgraph,
            heuristic=heuristic,
            pruned_universe=pruned_universe_gnn,
            budget=budget,
            depth=len(pruned_universe_gnn),
            GNNpruner=None,
            train=False)
